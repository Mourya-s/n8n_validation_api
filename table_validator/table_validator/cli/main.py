...
def info() -> None:
    ...
    typer.echo(
        ...
        "     When a single source and target table are both named (not a "
        "schema-wide sweep), the wizard also offers these optional steps "
        "- answer 'no'/leave blank to skip any of them and keep today's "
        "plain full-table comparison:\n"
        "       - Primary key: a real key column for row-level comparison "
        "instead of the ROW_NUMBER() fallback.\n"
        "       - Customize column validation: compare ONLY specific "
        "columns, SKIP specific columns entirely, or ignore datatype "
        "mismatches on specific columns.\n"
        "       - Filter to a subset of rows (Databricks source only): "
        "restrict the whole comparison to rows matching a condition, e.g. "
        "\"id > 10 and id < 30\" or \"gender = 'male'\" - either the same "
        "condition on both sides, or a different one per side.\n"
        "       - Map renamed columns (Databricks source only): pair up "
        "individual columns that don't share a name between source and "
        "target.\n"
        "     When a schema is named on both sides but no specific table "
        "is (a schema-wide sweep, Databricks source only), the wizard "
        "instead offers to restrict the sweep to specific table names, or "
